...
class AISuggestionProvider extends ChangeNotifier {
    ...
    try {
      ...
      if (rawSuggestions.isEmpty) {
        ...
        final imageResults = await Future.wait(
          rawSuggestions.map((json) => _fetchWikipediaImage(
                (json['name'] ?? '').toString(),
              )),
        );
        ...
        for (var i = 0; i < rawSuggestions.length; i++) {
          ...
          final imageUrl = imageResults[i].isNotEmpty
              ? imageResults[i]
              : _getImageForPlace(
                  (json['name'] ?? '').toString(),
  ...
  static String _getImageForPlace(String name, String category) {
    ...
    if (categoryImages.containsKey(normalizedCategory)) {
    ...
    if (n.contains('mirissa')) {
      return 'https://upload.wikimedia.org/wikipedia/commons/thumb/f/f1/Coconut_Tree_Hill%2C_Mirissa.jpg/800px-Coconut_Tree_Hill%2C_Mirissa.jpg';
    }
    if (n.contains('sigiriya')) {
      return 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4c/Sigiriya_%28Lion_Rock%29%2C_Sri_Lanka.jpg/800px-Sigiriya_%28Lion_Rock%29%2C_Sri_Lanka.jpg';
    }
    if (n.contains('ella')) {
      return 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/2e/Horton_Plains_World%27s_End_%282%29.jpg/800px-Horton_Plains_World%27s_End_%282%29.jpg';
    }
    if (n.contains('kandy')) {
      return 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c4/Sri_Dalada_Maligawa.jpg/800px-Sri_Dalada_Maligawa.jpg';
    }
